...
internal extension Atomic {
  ...
  internal func syncRead(_ work: ((Value) throws -> Void)) rethrows {
    try queue.sync {
      try work(_value)
    }
  }
  
  internal func syncRead<T>(_ work: ((Value) throws -> T)) rethrows -> T {
    return try queue.sync {
      return try work(_value)
    }
  }
  
  internal func asyncRead(_ work: @escaping ((Value) -> Void)) {
  ...
  internal func syncWrite(_ work: (inout Value) throws -> Void) rethrows {
    try queue.sync(flags: .barrier) {
      try work(&_value)
    }
  }
  
  internal func syncWrite<T>(_ work: (inout Value) throws -> T) rethrows -> T {
    return try queue.sync(flags: .barrier) {
      return try work(&_value)
    }
  }
  
  internal func asyncWrite(_ work: @escaping (inout Value) -> Void) {
    queue.async(flags: .barrier) { [weak self] in
      guard let self = self else { return }
      work(&self._value)
    }
